...
namespace CRUD.ClassesEntidades.SQL
{
    class SqlPowerUp
    {
        ...
        static public void Add(PowerUp powerUp)
        {
            // Imprime DEBUG para a consola, se DEBUG local e GERAL estiverem ativos
            if (DEBUG_LOCAL)
            {
                Console.WriteLine("Debug: SQLpowerUp - add() - <----Iniciar Query---->");
                Console.WriteLine("Debug: SQLpowerUp - add() - DBMS ATIVO: " + DBMS_ACTIVE);
            }
            ...
            try
            {
                ...
                using (DbConnection conn = OpenConnection())
                {
                    ...
                    using (MySqlCommand sqlCommand = ((MySqlConnection)conn).CreateCommand())
                    {
                        ...
                        sqlCommand.CommandText = "INSERT INTO powerup"
                            + "(Descri,ImageUrl,TimeCharge)"
                            + "VALUES(@descri, @imageUrl, @timeCharge);";
                        sqlCommand.Parameters.Add(new MySqlParameter("@descri", powerUp.Descri));
                        sqlCommand.Parameters.Add(new MySqlParameter("@imageUrl", powerUp.ImageUrl));
                        sqlCommand.Parameters.Add(new MySqlParameter("@timeCharge", powerUp.TimeCharge));

                        // Tenta Executar e Se diferente de 1, provoca excessão saltanto para o catch
                        if (sqlCommand.ExecuteNonQuery() != 1)
                        {
                            throw new InvalidProgramException("SQLpowerUp - add() - mysql: ");
                        }
                    }
        ...
        static public List<PowerUp> GetAll(int order)
        {
            ...
            if (DEBUG_LOCAL)
            {
                Console.WriteLine("Debug: SQLpowerUp - getAll() - <----Iniciar Query---->");
                Console.WriteLine("Debug: SQLpowerUp - getAll() - DBMS ATIVO: " + DBMS_ACTIVE);
            }
            ...
            try
            {
                ...
                using (DbConnection conn = OpenConnection())
                {
                    ...
                    using (MySqlCommand sqlCommand = new MySqlCommand())
                    {
                        ...
                        if (DEBUG_LOCAL)
                        {
                            Console.WriteLine("Debug: SQLpowerUp - getAll() - MYSQL - SQLcommand OK");
                        }
                        ...
                        using (MySqlDataReader reader = sqlCommand.ExecuteReader())
                        {
                            if (DEBUG_LOCAL)
                            {
                                Console.WriteLine("Debug: SQLpowerUp - getAll() - MYSQL - DATAREADER CRIADO");
                            }
                            ...
                            while (reader.Read())
                            {
                                if (DEBUG_LOCAL)
                                {
                                    Console.WriteLine("Debug: SQLpowerUp - getAll(): MYSQL - DATAREADER TEM REGISTOS!");
                                }
                                ...
                                listaPowerUps.Add(use);       //adiciona o obj à lista

                                //Debug para Output: Interessa ver o que está a sair do datareader
                                if (DEBUG_LOCAL)
                                {
                                    Console.WriteLine(
                                        "Debug: SQLpowerUp - getAll() - DataReader - MYSQL:"
            ...
            {
                Console.WriteLine("Erro: SQLpowerUp - getAll() - \n" + e.ToString());
                MessageBox.Show(
                    "SQLpowerUp - GetAll() - \n Ocorreram problemas com a ligação à Base de Dados: \n" + e.ToString(),
                    "SQLpowerUp - GetAll() - Catch",  // Título
                    MessageBoxButton.OK,            // Botões
        ...
        static public PowerUp Get(int id)
        {
            PowerUp powerup = null;

            if (DEBUG_LOCAL)
            {
                Console.WriteLine("Debug: SQLpowerUp - get() - <----Iniciar Query---->");
                Console.WriteLine("Debug: SQLpowerUp - get() - DBMS ATIVO: " + DBMS_ACTIVE);
            }
            ...
            try
            {
                ...
                using (DbConnection conn = OpenConnection())
                {
                    ...
                    using (MySqlCommand sqlCommand = new MySqlCommand())
                    {
                        ...
                        // SQL DDL
                        sqlCommand.CommandText = "SELECT * FROM PowerUp where Id=@Id;";
                        ...
                        using (MySqlDataReader reader = sqlCommand.ExecuteReader())
                        {
                            if (DEBUG_LOCAL)
                            {
                                Console.WriteLine("Debug: SQLpowerUp - get() - MYSQL - DataReader CRIADO: ");
                            }
                            ...
                            if (reader.Read())
                            {
                                if (DEBUG_LOCAL)
                                {
                                    Console.WriteLine("Debug: SQLpowerUp - get() MYSQL: DATAREADER TEM REGISTO!");
                                }
                                ...
                                {
                                    Console.WriteLine(
                                        "Debug: SQLpowerUp - get() - DataReader - MYSQL:"
                                        + "\n Id->" + reader.GetInt32(reader.GetOrdinal("Id")).ToString()
            ...
            {
                Console.WriteLine("Erro: SQLpowerUp - get() - \n" + e.ToString());
                MessageBox.Show(
                    "SQLpowerUp - Get() - \n Ocorreram problemas com a ligação à Base de Dados: \n" + e.ToString(),
                    "SQLpowerUp - Get() - Catch", // Título
                    MessageBoxButton.OK,        // Botões
                    MessageBoxImage.Error       // Icon
                );
                return null;
        ...
        /// </summary>
        /// <param name="powerup">Objeto com id a alterar da tabela</param>
        static public void Set(PowerUp powerup)
        {
            if (DEBUG_LOCAL)
            {
                Console.WriteLine("Debug: SQLpowerUp - set() - <----Iniciar Query---->");
                Console.WriteLine("Debug: SQLpowerUp - set() - DBMS ATIVO: " + DBMS_ACTIVE);
            }
            ...
            try
            {
                ...
                using (DbConnection conn = OpenConnection())
                {
                    ...
                    {
                        sqlCommand.CommandType = CommandType.Text;
                        sqlCommand.CommandText = "UPDATE powerup SET "
                        + " Descri = @descri,"
                        + " ImageUrl = @imageUrl,"
                        + " TimeCharge = @timeCharge"
                        + " WHERE Id = @id;";
                        sqlCommand.Parameters.Add(new MySqlParameter("@id", powerup.Id));
                        sqlCommand.Parameters.Add(new MySqlParameter("@descri", powerup.Descri));
                        sqlCommand.Parameters.Add(new MySqlParameter("@imageUrl", powerup.ImageUrl));
                        sqlCommand.Parameters.Add(new MySqlParameter("@timeCharge", powerup.TimeCharge));
                        ...
                        if (sqlCommand.ExecuteNonQuery() != 1)
                        {
                            ...
                            throw new InvalidProgramException("SQLpowerUp - set() - mysql: ");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine("Erro: SQLpowerUp - set() - \n" + e.ToString());
                MessageBox.Show(
                    "SQLServer - Set() - \n Ocorreram problemas com a ligação à Base de Dados: \n" + e.ToString(),
        ...
        /// </summary>
        /// <param name="powerup">Objeto com id a apagar da tabela</param>
        static public void Del(PowerUp powerup)
        {
            if (DEBUG_LOCAL)
            {
                Console.WriteLine("Debug: SQLpowerUp - del() - <--Iniciar Query-->");
                Console.WriteLine("Debug: SQLpowerUp - del() - DBMS ATIVO: " + DBMS_ACTIVE);
            }
            ...
            try
            {
                ...
                using (DbConnection conn = OpenConnection())
                {
                    using (MySqlCommand sqlCommand = ((MySqlConnection)conn).CreateCommand())
                    {
                        // Prepara e executa o SQL DML
                        sqlCommand.CommandType = CommandType.Text;
                        sqlCommand.CommandText = "DELETE FROM powerup WHERE Id=@id;";
                        sqlCommand.Parameters.Add(new MySqlParameter("@id", powerup.Id));
                        ...
                        {
                            // Se diferente, inverte o commit e Provoca a excessão saltanto para o catch
                            throw new InvalidProgramException("SQLpowerUp - del() - mysql: ");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine("Erro: SQLpowerUp - del() - \n" + e.ToString());
                MessageBox.Show(
                    "SQLpowerUp - Del() - \n Ocorreram problemas com a ligação à Base de Dados: \n" + e.ToString(),
                    "SQLpowerUp - Del() - Catch", // Título
                    MessageBoxButton.OK,        // Botões
                    MessageBoxImage.Error       // Icon
                );
            }
        ...
        /// </summary>
        /// <param name="powerup">Registo a testar</param>
        /// <returns></returns>
        static public bool CheckRelationalIntegrityViolation(PowerUp powerup)
        {
            if (DEBUG_LOCAL)
            {
                Console.WriteLine("Debug: SQLpowerUp - checkRelationalIntegrityViolation() - <----Iniciar Query---->");
                Console.WriteLine("Debug: SQLpowerUp - checkRelationalIntegrityViolation() - DBMS ATIVO: " + DBMS_ACTIVE);
            }
            ...
            bool relationalViolationForFKtables = false;   // ativa-se quando o powerup é fk em tabelas relacionadas
